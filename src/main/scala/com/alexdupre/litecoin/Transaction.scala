...
import com.alexdupre.litecoin.Script.Runner
import scodec.bits.ByteVector

import scala.collection.mutable.ArrayBuffer

object OutPoint extends LtcSerializer[OutPoint] {
...
/**
  ...
  */
case class OutPoint(hash: ByteVector32, index: Long) extends LtcSerializable[OutPoint] {
  require(index >= -1)
  ...
  override def serializer: LtcSerializer[OutPoint] = OutPoint
}

object TxIn extends LtcSerializer[TxIn] {
...
/**
  ...
  */
case class TxIn(outPoint: OutPoint, signatureScript: ByteVector, sequence: Long, witness: ScriptWitness = ScriptWitness.empty) extends LtcSerializable[TxIn] {
  def isFinal: Boolean = sequence == TxIn.SEQUENCE_FINAL

  def hasWitness: Boolean = witness.isNotNull

  override def serializer: LtcSerializer[TxIn] = TxIn
}

object TxOut extends LtcSerializer[TxOut] {
  ...
  override def validate(input: TxOut): Unit = {
    ...
    require(amount.toLong <= LtcAmount.MaxMoney, s"invalid txout amount: $amount")
...
/**
  ...
  */
case class TxOut(amount: Satoshi, publicKeyScript: ByteVector) extends LtcSerializable[TxOut] {
  override def serializer: LtcSerializer[TxOut] = TxOut
}

object ScriptWitness extends LtcSerializer[ScriptWitness] {
...
/**
  ...
  */
case class ScriptWitness(stack: Seq[ByteVector]) extends LtcSerializable[ScriptWitness] {
  def isNull = stack.isEmpty

  def isNotNull = !isNull

  override def serializer: LtcSerializer[ScriptWitness] = ScriptWitness
}

object Transaction extends LtcSerializer[Transaction] {
  ...
  override def validate(input: Transaction): Unit = {
    ...
    require(input.txOut.map(_.amount.toLong).sum >= 0, "sum of outputs amount is invalid")
    require(input.txOut.map(_.amount.toLong).sum <= LtcAmount.MaxMoney, "sum of outputs amount is invalid")
...
/**
  ...
  */
case class Transaction(version: Long, txIn: Seq[TxIn], txOut: Seq[TxOut], lockTime: Long) extends LtcSerializable[Transaction] {
  ...
  override def serializer: LtcSerializer[Transaction] = Transaction
}
