...
class CheckLockTimeVerifySpec extends FlatSpec {
  "Bip65" should "let you initiate payment channels" in {
    ...
    val tx = {
      ...
      val sig = Transaction.signInput(tmpTx, 0, previousTx.txOut(0).publicKeyScript, SIGHASH_ALL | SIGHASH_FORKID, previousTx.txOut(0).amount, key)
    ...
    val tx1 = {
      ...
      val sig = Transaction.signInput(tmpTx, 0, Script.write(scriptPubKey), SIGHASH_ALL | SIGHASH_FORKID, 100 sat, keyAlice)
    ...
    val tx2 = {
      ...
      val sig1 = Transaction.signInput(tmpTx, 0, Script.write(scriptPubKey), SIGHASH_ALL | SIGHASH_FORKID, 100 sat, keyAlice)
      val sig2 = Transaction.signInput(tmpTx, 0, Script.write(scriptPubKey), SIGHASH_ALL | SIGHASH_FORKID, 100 sat, keyBob)
