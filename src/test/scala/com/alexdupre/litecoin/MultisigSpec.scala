...
class MultisigSpec extends FunSuite with Matchers {
  ...
  test("create and sign multisig transactions") {
    ...
    val txIn = TxIn(
      OutPoint(ByteVector32(hex"7c69ae71a68c0b76672ec008bb2d81d79985019609862dee3f807b4c2344f4ef".reverse), 1),
      signatureScript = ByteVector.empty, // empy signature script
      sequence = 0xFFFFFFFFL)

    // and we want to sent the output to our multisig address
    val txOut = TxOut(
      amount = 900000 sat, // 0.009 LTC) satoshi, meaning the fee will be 0.01-0.009 = 0.001
