...
class SegwitSpec extends FunSuite {
  ...
  test("create p2wpkh tx") {
    val priv1 = PrivateKey.fromBase58("cQWAJTf9HLiqLm6bjPViJTGAVc6zWvxmoE1ekjR4WjNZqut1yqrz", Base58.Prefix.SecretKeyTestnet)._1
    ...
    val tx2 = {
      val tmp = Transaction(version = 1,
        txIn = TxIn(OutPoint(tx1.hash, 1), sequence = 0xffffffffL, signatureScript = ByteVector.empty, witness = ScriptWitness.empty) :: Nil,
    ...
    Transaction.correctlySpends(tx2, Seq(tx1), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
    assert(tx2.txid == ByteVector32(hex"5e61b8897429c3014bb837c0bac1cc6782b0f7d5281127cef37c1dfad6d2d924"))
    ...
    assert(tx3.txid == ByteVector32(hex"6af95097a6157ca1a90fadf4fe417144b5e86247f363990c3fed066ad71038f4"))
  ...
  test("create p2wsh tx") {
    ...
    val priv2 = PrivateKey.fromBase58("cTnvMLGakZXFpsX3st5kJn87YAJtLMNPmoEioUemtiJJgbF6dpHj", Base58.Prefix.SecretKeyTestnet)._1
    val pub2 = priv2.publicKey

    val priv3 = PrivateKey.fromBase58("cW1EgBmugeNPWdH8xZzohCwkiP965zoy4VCE8UNZqq6epWKeaCu3", Base58.Prefix.SecretKeyTestnet)._1
    ...
    Transaction.correctlySpends(tx2, Seq(tx1), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
    assert(tx2.txid == ByteVector32(hex"852e99626ab08c8275237c365e4f74db70bb649f1b6b03bc15dd7455f2f0f059"))
    ...
    Transaction.correctlySpends(tx3, Seq(tx2), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
    assert(tx3.txid == ByteVector32(hex"03b9f14d03b3310dbc6da003aac6e1ac18558770979823b5fc3977821affa125"))
  ...
  test("create p2pkh embedded in p2sh") {
    ...
    val tx1 = {
      val tmp: Transaction = Transaction(version = 1,
        ...
        txOut = TxOut(0.49 ltc, OP_0 :: OP_PUSHDATA(Crypto.hash160(pub1.value)) :: Nil) :: Nil,
        lockTime = 0
      )
      val pubKeyScript = Script.pay2pkh(pub1)
    ...
    assert(tx1.txid === ByteVector32(hex"4b74ab021f51c7af226aebfa10d8b84e2bb4252ea7db93a32b181b50cdd1bd79"))
